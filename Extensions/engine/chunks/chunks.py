...
class ChunkOX:
    ...
    def generate_chunk(self, height: int or float):
        height = min(height, ChunkOX.CHUNK_SIZE.y)
        chunk_data = zeros(0)

        for x in range(ChunkOX.CHUNK_SIZE.x):
            for y in range(ChunkOX.CHUNK_SIZE.y):
                for z in range(ChunkOX.CHUNK_SIZE.z):
                    if y < height:
    ...
    def grid_position(self):
        return self.position // ChunkOX.CHUNK_SIZE

    @staticmethod
    def data_index_to_position(index):
        x = index // ChunkOX.CHUNK_SIZE_yz
        y = index // ChunkOX.CHUNK_SIZE.z
        z = index % ChunkOX.CHUNK_SIZE_yz
        ...
        for x in range(ChunkOX.CHUNK_SIZE.x):
            for y in range(ChunkOX.CHUNK_SIZE.y):
                for z in range(ChunkOX.CHUNK_SIZE.z):
                    ...
                    block_type = self.chunk_data[block_position.x * ChunkOX.CHUNK_SIZE_yz + block_position.y * ChunkOX.CHUNK_SIZE.z + block_position.z]
                    if block_type:
                    ...
                    for face_index in range(0, 6):
                        ...
                        if 0 <= check_position.x < ChunkOX.CHUNK_SIZE.x and 0 <= check_position.y < ChunkOX.CHUNK_SIZE.y and 0 <= check_position.z < ChunkOX.CHUNK_SIZE.z:
                            block_data = self.chunk_data[check_position.x * ChunkOX.CHUNK_SIZE_yz + check_position.y * ChunkOX.CHUNK_SIZE.z + check_position.z]
...
class chunks:
    ...
    def load(position: tuple[3] or list[3], atlas):
        ...
        chunk = ChunkOX(atlas, position * ChunkOX.CHUNK_SIZE)
